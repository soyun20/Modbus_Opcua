...
uint8_t getSendMsg[MODBUS_TCP_MAX_ADU_LENGTH];
int getSendLen;
uint8_t getReceiveMsg[MODBUS_TCP_MAX_ADU_LENGTH];
int getReceiveLen;
int invalid_fuc = 0;
int invalid_len = 0;
int byte_swap = 0;
...
int get_msgLen(char *check){
    if(!strcmp(check,"INPUT")){
        return getSendLen;
    }
    else if (!strcmp(check,"OUTPUT"))
        return getReceiveLen;
}
uint8_t* get_msg(char *check){
    if(!strcmp(check,"INPUT"))
        return getSendMsg;
    else if (!strcmp(check,"OUTPUT"))
...
void Invalid_Length(void){
    invalid_len = 1;
}
int Byte_Swap(int no){
    byte_swap = no;
    return byte_swap;
}
int Word_Swap(int no){
    word_swap = no;
    return word_swap;
}
void Byte_Swap_Func(uint8_t* msg, int idx, int msg_length) {
    uint8_t tmp = msg[msg_length + idx];
    msg[msg_length + idx] = msg[msg_length + idx+1];
    msg[msg_length + idx+1] = tmp;
}
void Word_Swap_Func(uint8_t* msg, int idx, int msg_length) {
    uint8_t tmp1 = msg[msg_length + idx];
    uint8_t tmp2 = msg[msg_length + idx+1];
    msg[msg_length + idx] = msg[msg_length + idx+2];
    msg[msg_length + idx+1] = msg[msg_length + idx+3];
    msg[msg_length + idx+2] = tmp1;
    msg[msg_length + idx+3] = tmp2;
}
...
static int send_msg(modbus_t *ctx, uint8_t *msg, int msg_length)
{
    ...
    getSendLen = msg_length;
    memcpy(getSendMsg, msg, msg_length);
...
int _modbus_receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    ...
    while (length_to_read != 0) {
        ...
        if(msg_len_count==2){
            ...
            if(byte_swap==1 && length_to_read>=2){
                for(unsigned int idx=0;idx<length_to_read;idx+=2){
                    Byte_Swap_Func(msg, idx, msg_length);
                    printf("length: %d \n",msg_length);
                }
            }
            if(word_swap==1){
                if(length_to_read%4!=0){
                    unsigned int idx;
                    for(idx=0;idx<length_to_read-2;idx+=4){
                        Word_Swap_Func(msg, idx, msg_length);
                    }
                    msg[msg_length + idx] = 0;
                    msg[msg_length + idx+1] = 0;
                }
                else{
                    for(unsigned int idx=0;idx<length_to_read;idx+=4){
                        Word_Swap_Func(msg, idx, msg_length);
                    }
                }
            }
        ...
        getReceiveLen = msg_length;
        memcpy(getReceiveMsg, msg, msg_length);
