...
void * Client::ThreadFunction(void *arg) {
    ...
    ThreadArgs_client* targs = (ThreadArgs_client*) arg;
    std::string id = targs->id;
    ClientMQTT Send_Log = targs->Send_Log;
    ...
    std::cout << "걸린 시간2: " << elapsed_seconds.count() << " 초\n";

    client.clientId = id;
    client.Send_Log = Send_Log;
    modbus_t* ctx = client.Connection_Initialize();


    end = std::chrono::steady_clock::now();
    elapsed_seconds = end - client_start_time; // 시간 차이 계산
    std::cout << "걸린 시간3: " << elapsed_seconds.count() << " 초\n";
    ...
    elapsed_seconds = end - client_start_time; // 시간 차이 계산
    std::cout << "걸린 시간4: " << elapsed_seconds.count() << " 초\n";
    ...
    elapsed_seconds = end - client_start_time; // 시간 차이 계산
    std::cout << "걸린 시간5: " << elapsed_seconds.count() << " 초\n";
...
///////////////////////////////////////////////////////////
    ...
    for (int i = 0; i < root["networkData"]["msgCount"].asInt(); ++i) {
        ...
        // }
        // std::cout << "@@@@@@@@@@@@@@@@@@@: " << combinedJson["msgData"][0]["values"]  << std::endl;
        // combinedJson["msgData"][0]["values"] = std::to_string(std::stoi(combinedJson["msgData"][0]["values"])+k);
		...
		for (auto &key : combinedJson["msgData"].getMemberNames()) {
            ...
            // if (key == "values")
            // {
            //     std::cout << "@@@@@@@@@@@@@@@@@@@: " << mergedJson  << std::endl;
            //     // mergedJson[key][0] = std::to_string(std::stoi(mergedJson[key][0])+k);
            //     std::string currentString = mergedJson[key][0].asString();
            //     int currentValue = stoi(currentString);
            //     mergedJson[key][0] = std::to_string(currentValue + k);
            //     std::cout << "$$$$$$$$$$$$$$$$$$$: " << mergedJson  << std::endl;
    ...
    auto end = std::chrono::steady_clock::now();
    std::chrono::duration<double> elapsed_seconds = end - client_start_time; // 시간 차이 계산
    std::cout << "걸린 시간1: " << elapsed_seconds.count() << " 초\n";

    for (int i = 0; i < root["networkData"]["msgCount"].asInt(); ++i) {
        origin_root = jsonArray[i];
        ////////////////////////////////////
        // std::cout << "#####################: " << origin_root  << std::endl;
        pthread_t newThread;
        
        ThreadArgs_client* args = new ThreadArgs_client {clientId, Send_Log};
        pthread_create(&newThread, nullptr, ThreadFunction, args);
        threads.insert({clientId, newThread});
        numThreads++;
        sleep(1);
    }
    ...
    // auto end = std::chrono::steady_clock::now();
    // std::chrono::duration<double> elapsed_seconds = end - start; // 시간 차이 계산
    // std::cout << "걸린 시간: " << elapsed_seconds.count() << " 초\n";
...
int Client::start(Json::Value* setting, std::string ID)
{
    ...
    // close_client._sub_client_id = ID+"_sub_client_closecluient";
    // close_client.Subscribe_Initialize();
    // Send_Log._pub_client_id = ID+"_pub_client_sendlog";
    ...
    while(1)
    {
        ...
        if(close_client.is_exit == ID){
            ...
            Send_Log.Uninitialize();
            close_client.Uninitialize();
...
Json::Value Client::mqtt_client(int mqtt_slave_id)
{
    ...
    client_MQTT._sub_client_id = std::to_string(mqtt_slave_id)+clientId+"_sub_client";
    client_MQTT._pub_client_id = std::to_string(mqtt_slave_id)+clientId+"_pub_client";
