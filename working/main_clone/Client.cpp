...
void Client::ReadCoils(modbus_t *ctx)
{
    ...
    rc = modbus_read_bits(ctx, address, quantity, tab_rp_bits);
    if (rc == -1) {
        fprintf(stderr, "Read Coils error: %s\n", modbus_strerror(errno));
    } else {
        for (int i = 0; i < quantity; i++) {
            printf("Coil %d: %d\n", address+i, tab_rp_bits[i]);
        }
    }
...
void Client::ReadDisInputs(modbus_t *ctx)
{
    ...
    rc = modbus_read_input_bits(ctx, address, quantity, tab_rp_bits);
    if (rc == -1) {
...
void Client::ReadHoldingRegs(modbus_t *ctx)
{
    ...
    rc = modbus_read_registers(ctx, address, quantity, tab_rp_registers);
    if (rc == -1) {
        fprintf(stderr, "Read Holding Registers error: %s\n", modbus_strerror(errno));
    } else {
        for (int i = 0; i < quantity; i++) {
            printf("Holding Register %d: %d\n", address+i, tab_rp_registers[i]);
        }
    }
    write_log_setting("CLIENT_INPUT","/modbus/log");
    write_log_setting("CLIENT_OUTPUT","/modbus/log");
...
void Client::ReadInputRegs(modbus_t *ctx)
{
    ...
    rc = modbus_read_input_registers(ctx, address, quantity, tab_rp_registers);
    if (rc == -1) {
...
void Client::WriteCoil(modbus_t *ctx)
{
    ...
    rc = modbus_write_bit(ctx, address, values[0]);
    if (rc == -1) {
...
void Client::WriteReg(modbus_t *ctx)
{
    ...
    rc = modbus_write_register(ctx, address, values[0]);
    if (rc == -1) {
...
void Client::WriteMultiCoils(modbus_t *ctx)
{
    ...
    uint8_t* tab_value = reinterpret_cast<uint8_t *>(values.data());

    rc = modbus_write_bits(ctx, address, values.size(), tab_value);
    if (rc == -1) {
...
void Client::WrtieMultiRegs(modbus_t *ctx)
{
    ...
    rc = modbus_write_registers(ctx, address, values.size(), tab_value);
    if (rc == -1) {
...
void Client::WriteMaskReg(modbus_t *ctx)
{
    ...
    rc = modbus_write_register(ctx, address, 18);
    rc = modbus_mask_write_register(ctx, address, andMask, orMask);
    rc = modbus_read_registers(ctx, address, 1, tab_rp_registers);
    if (rc == -1) {
...
void Client::write_log_setting(char* type, char* topic){
    uint8_t* query;
    int length;
...
void Client::write_log(char* type, char* topic, char* content){
